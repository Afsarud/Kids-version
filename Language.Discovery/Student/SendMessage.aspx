...
<![endif]-->
		...
		function InitializeFreeMessage() {
			...
			buttonsOpts[translations["Ok"]] = function () {
                ...
                checkMessages('freemessage')
					.then(function () {
                        let numberOfExistingQuestion = 0;
						$('#divFinalNativeMessage > .palette > .column1 > .firstword, #divFinalNativeMessage > .palette > .column1 > span').each(function () {
                            if (isQuestion($(this).text())) {
                                numberOfExistingQuestion++;
                            }
                        });

                        //if (isQuestion($("#txtFreeMessage1").val() + $("#txtFreeMessage2").val())) {
                        const count = questionMarkCount($("#txtFreeMessage1").val());
						numberOfExistingQuestion = numberOfExistingQuestion + count;
                        //}
                        
						
                        //}

                        if (isQuestionExceeded(numberOfExistingQuestion)) {
                            return;
                        }
   ...
   //         }
        ...
        function questionMarkCount(str) {
			const specialChars = /[?？]/gm;
			const count = (str.match(specialChars) || []).length;
			return count;
        }

		function isQuestionExceeded(numberOfExistingQuestion) {
			if (numberOfExistingQuestion > 2) {
				toastr.clear();
				toastr.options = {
					"closeButton": true,
					"debug": false,
				...
				toastr.error($('#hdnQuestionLimit').val(), 'Error');
				return true;
			} else {
				return false;
			}
		...
		function AddAllWords(el) {
			...
			$('#divFinalNativeMessage > .palette > .column1 >.firstword, #divFinalNativeMessage > .palette > .column1 > span').each(function () {
                numberOfExistingQuestion = numberOfExistingQuestion + questionMarkCount($(this).text())
			});

			$(el).siblings("ul").find("li").each(function () {
				let div = $(this).find('div')[0];
				$(div).find(".firstword").each(function () {
					if (isQuestion($(this).text())) {
						numberOfExistingQuestion++;
					}
				});
	...
	<div style="width:100%;">
		...
		<asp:HiddenField ID="hdnQuestionLimit" runat="server" ClientIDMode="Static" meta:resourcekey="hdnQuestionLimitResource1"/>
		...
		<div class="sendmsg_top_container" id="sendmsg_top_container">
			<div class="stc_avatar_container" id="stc_avatar_container">
                ...
                <div id="divCCContainer">
                    <asp:CheckBox ID="chkCC" runat="server" ClientIDMode="Static"/>
				...
				<div class="smsc_right" id="smsc_right">
					<div class="smsc_btnsend_count" id="smsc_btnsend_count">
                        ...
                        <div id="saveContainer">
                            <img src="../Images/new/mailSave.png" id="mailSave" onclick="save(true);" />
