...
GO
/****** Object:  StoredProcedure [dbo].[usp_DiscoverNewFriends]    Script Date: 7/11/2023 8:58:26 PM ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	IF(@LevelID = 3)
	begin
		exec [usp_DiscoverNewFriends_Youth] @InterestXml, @CityID, @Name,@SearcheeID,@GenderXml, @IsAddUser
		return ;
	end
	...
	declare @UserWithDirectReply as table
	(
		UserID int
	)
	...
	INSERT INTO @UserWithDirectReply
	select RecepientID from UserMessage where RecepientID in (select UserId from @UserMessage) AND IsDirectReply = 1 group by RecepientID
	...
	and u.CountryID = @UserCountryId AND UM.CreateDate>=DATEADD(DAY,-7,GETDATE())
	--GROUP BY U.USerID
	...
	INNER JOIN [user] u on UM.SenderID = u.UserID
	WHERE  ((@LevelID <> 0 and u.levelid = @levelID)  OR U.LevelID not in (3,4))
	and u.CountryID = @UserLearningCountryId AND UM.CreateDate>=DATEADD(DAY,-7,GETDATE())
	...
	BEGIN
		...
		(
			...
			(
				...
				AND (([User].LastLogin is not null AND Datediff(day,[User].LastLogin, getdate()) <= 5000))) OR [User].IsRobot = 1 )
	...
	BEGIN
		...
		(
			...
			(
				...
				AND (charindex('@', @Name) > 0 OR ([User].IsActive = 1 
				AND (([User].LastLogin is not null AND Datediff(day,[User].LastLogin, getdate()) <= 5000)))
					OR [User].IsRobot = 1 ) 
				AND (isnull(@Name,'') = '' or ([User].UserName like '%' + @Name + '%' or [User].FirstName like '%' + @Name + '%'))
				AND (isnull([User].IsDemo,0) = @IsDemo ) 
				and isnull([User].IsRobot,0) = @SchoolTypeDemo
	...
	DELETE @TempUser WHERE UserID IN (SELECT UserID FROM @UserWithDirectReply)
	DELETE @TempUserZero WHERE UserID IN (SELECT UserID FROM @UserWithDirectReply)
	...
	FROM @TempUserZero U
	WHERE U.UserID not in (SELECT UserID from CTE Where DuplicateCount = 1 )
