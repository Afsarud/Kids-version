...
namespace Language.Discovery.Repository
{
    public class UserRepository : IRepository<UserContract>
    {
        ...
        public List<UserContract> SearchUserForMessagePool(SearchUserDTO tObject)
        {
            try
            {
                List<UserContract> users = null;
                ...
                DataSet ds = DatabaseHelper.ExecuteQuery("usp_SearchUserAdminWithoutDirectReply", paramlist);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    users = (List<UserContract>)CollectionHelper.ConvertTo<UserContract>(ds.Tables[0]);
        ...
        public bool UpdateUserParentsInfo(long userId, string parentsName, string childFirstname, string email)
        {
            ...
            try
            {
                List<Parameter> paramlist = new List<Parameter>();
                paramlist.Add(new Parameter("UserID", userId));
                paramlist.Add(new Parameter("ChildsFirstName", childFirstname));
                paramlist.Add(new Parameter("ParentsName", parentsName));
                paramlist.Add(new Parameter("ParentsEmail", email));
                int affected = DatabaseHelper.ExecuteNonQuery("usp_UpdateChildsUserParentsInfo", paramlist);
